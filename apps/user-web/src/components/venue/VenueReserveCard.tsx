...
// ─── Airbnb-style date input ──────────────────────────────────────────────────

function DateField({
  label, value, placeholder, onClick, borderRight = false,
}: {
  label: string; value: string | null; placeholder: string; onClick?: () => void; borderRight?: boolean
}) {
  return (
    <button
      type="button"
    ...
    >
      ...
      <p className={`mt-0.5 text-sm font-semibold truncate ${value ? 'text-zinc-900' : 'text-zinc-400'}`}>
        {value ?? placeholder}
      </p>
    </button>
  )
...
export function VenueReserveCard({
  venue, bookingType, showTypeToggle,
  startDate, endDate, selectedStart, selectedEnd,
  quote, quoteLoading, quoteError, slotError, isPending,
  onBookingTypeChange, onReset, onBook,
}: Props) {
  const startLabel = startDate ? formatDate(startDate + 'T00:00:00') : null
  const endLabel   = endDate   ? formatDate(endDate   + 'T00:00:00') : null
  const timeLabel  = selectedStart && selectedEnd ? `${formatTime(selectedStart)} – ${formatTime(selectedEnd)}` : null
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white shadow-lg overflow-hidden">
      ...
      <div className="px-6 pt-6 pb-5 border-b border-zinc-100">
        {/* If we have a total from the quote, show it prominently */}
        {totalLabel && !quoteLoading ? (
          <div>
            ...
            {priceDisplay && (
              <p className="mt-0.5 text-xs text-zinc-400">{priceDisplay.amount} / {priceDisplay.unit}</p>
            )}
          </div>
        ) : priceDisplay ? (
          <div className="flex items-baseline gap-1.5">
      ...
      {showTypeToggle && (
        <div className="px-6 pt-4">
          <div className="flex overflow-hidden rounded-xl border border-zinc-200">
            <button type="button" onClick={() => onBookingTypeChange('full_day')}
              className={`flex-1 py-2.5 text-sm font-medium transition-colors ${
                bookingType === 'full_day' ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-500 hover:bg-zinc-50'
              }`}>
              Full Day
            </button>
            <button type="button" onClick={() => onBookingTypeChange('time_slot')}
              className={`flex-1 border-l border-zinc-200 py-2.5 text-sm font-medium transition-colors ${
                bookingType === 'time_slot' ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-500 hover:bg-zinc-50'
              }`}>
              Time Slot
            </button>
          </div>
      ...
      <div className="px-6 pb-6">
        ...
        >
          {isPending ? (
            ...
            'Select end date to continue'
          ) : bookingType === 'time_slot' && !hasSlot ? (
            'Select a time to continue'
          ) : quoteLoading ? (
            <span className="flex items-center justify-center gap-2"><Spinner /> Calculating price…</span>
          ) : advanceLabel ? (
